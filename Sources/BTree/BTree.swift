...
extension BTree {
  ...
  /// - Complexity: O(log(`count`))
  @discardableResult
  public mutating func insertOrReplace(_ element: Element, at selector: BTreeKeySelector = .any)
    -> Element?
  {
  ...
  /// - Complexity: O(log(`count`))
  @discardableResult
  public mutating func insertOrFind(_ element: Element, at selector: BTreeKeySelector = .any)
    -> Element?
  {
    let selector = (selector == .after ? .last : selector)
    makeUnique()
      ...
      descend: { node in
        ...
        if node.isLeaf {
          if let m = slot.match {
            ...
            old = node.remove(
              slot: slot.descend == node.elements.count ? slot.descend - 1 : slot.descend
            )
          }
          return nil
        }
