...
extension BTreeNode {
  func assertValid(file: StaticString = #file, line: UInt = #line) {
    func testNode(level: Int, node: BTreeNode<Key, Value>, minKey: Key?, maxKey: Key?) -> (
      count: Int, defects: [String]
    ) {
      var defects: [String] = []
      ...
      if node.elements.count != node.children.count - 1 {
        defects.append(
          "Mismatching item counts in internal node (elements.count: \(node.elements.count), children.count: \(node.children.count)"
        )
      }
...
func uniformNode(depth: Int, order: Int, keysPerNode: Int, offset: Int = 0) -> BTreeNode<
  Int, String
> {
  ...
  let tree = BTree<Int, String>(
    sortedElements: sequence,
    order: order,
    fillFactor: Double(keysPerNode) / Double(order - 1)
  )
  return tree.root
}
