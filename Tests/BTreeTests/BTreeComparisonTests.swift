...
private func makeTree<S: Sequence>(_ s: S, order: Int = 5, keysPerNode: Int? = nil) -> Tree
where S.Element == Int {
...
class BTreeComparisonTests: XCTestCase {
  ...
  func test_isSubsetOf_Subtrees() {
    ...
    tree.forEachSubtree { subtree in
      ...
      XCTAssertEqual(
        subtree.isStrictSubset(of: tree, by: .groupingMatches),
        subtree.count != tree.count
      )
      XCTAssertEqual(
        subtree.isStrictSubset(of: tree, by: .countingMatches),
        subtree.count != tree.count
      )
  ...
  func test_isSupersetOf_Subtrees() {
    ...
    tree.forEachSubtree { subtree in
      ...
      XCTAssertEqual(
        subtree.isSuperset(of: tree, by: .groupingMatches),
        subtree.count == tree.count
      )
      XCTAssertEqual(
        subtree.isSuperset(of: tree, by: .countingMatches),
        subtree.count == tree.count
      )

      XCTAssertEqual(
        tree.isStrictSuperset(of: subtree, by: .groupingMatches),
        subtree.count != tree.count
      )
      XCTAssertEqual(
        tree.isStrictSuperset(of: subtree, by: .countingMatches),
        subtree.count != tree.count
      )
  ...
  func test_isSubsetOf_LongDuplicateKeyRuns() {
    let a1 = makeTree(
      Array(repeating: 0, count: 20) + Array(repeating: 1, count: 100)
    ...
    let b = makeTree(
      Array(repeating: 0, count: 20) + Array(repeating: 1, count: 99)
        + Array(repeating: 2, count: 30)
    )
    let c = makeTree(
      Array(repeating: 0, count: 20) + Array(repeating: 1, count: 101)
        + Array(repeating: 2, count: 30)
    )
