...
class BTreeNodeTests: XCTestCase {
  ...
  func testInterruptibleForEach() {
    ...
    var i = 0
    XCTAssertTrue(
      node.forEach { (key, value) -> Bool in
        XCTAssertEqual(key, i)
        XCTAssertEqual(value, String(i))
        i += 1
        return true
  ...
  func testFixDeficiencyByRotatingLeft() {
    ...
    let c = node.count
    node.children[2].insert(
      node.setElement(inSlot: 1, to: node.children[1].remove(slot: 1)),
      inSlot: 0
    )
  ...
  func testMoreJoin() {
    ...
    checkNode(
      Node.join(left: createNode(0...0), separator: (1, "1"), right: createNode(2..<3)),
      0..<3
    )

    checkNode(
      Node.join(left: createNode(0...98), separator: (99, "99"), right: createNode(100..<101)),
      0..<101
    )
    checkNode(
      Node.join(left: createNode(0...0), separator: (1, "1"), right: createNode(2..<101)),
      0..<101
    )
    checkNode(
      Node.join(left: createNode(0...99), separator: (100, "100"), right: createNode(101..<200)),
      0..<200
    )
